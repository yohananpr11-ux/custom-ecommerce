...
// prior incident found that running `npm run build` locally with no
// PRERENDER_API_BASE set silently made a real, unauthenticated GET request
// to the live production backend -- the previous design was deny-list-based
// (anything not explicitly hermetic fell through to production); this is
// now allow-list-based (only a recognized deploy platform, or an explicit
// override, may ever use the production URL). Local/offline runs still
// degrade gracefully to products_fallback.json (see below) -- they simply
// never attempt the network call that would otherwise silently reach
// production.
const ON_KNOWN_DEPLOY_PLATFORM = process.env.VERCEL === '1' || process.env.RENDER === 'true';
const HAS_EXPLICIT_API_BASE = Boolean(process.env.PRERENDER_API_BASE && process.env.PRERENDER_API_BASE.trim());
...
async function main() {
  ...
  if (REQUIRE_API) {
    ...
    // except the fetch attempt itself is skipped entirely (never even
    // constructs a request) when there is no usable API base at all.
    products = [];
    if (!API_BASE) {
      console.warn('  ⚠ No usable API base (not on a recognized deploy platform, and PRERENDER_API_BASE not set) -- refusing to fall back to any hardcoded production URL. Trying fallback file...');
    } else {
      try {
        products = await fetchProducts();
      } catch (err) {
