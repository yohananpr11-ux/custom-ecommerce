    ...
    try {
      ...
      for (const p of products) {
            ...
            if (rows) {
              ...
              db.all(`SELECT id FROM products WHERE type = 'printify' AND printifyId = ?`, [printifyId], (err2, allMatches) => {
                ...
                db.run(`UPDATE products SET title = ?, price = ?, imageUrl = ?, backImageUrl = ?, images = ?, description = ?, printifyId = ?, fabric = ?, careInstructions = ?, deliveryInfo = ? WHERE id = ?`,
                  [title, retailPrice, frontImageUrl, backImageUrl, JSON.stringify({ allImages, variantImageMap }), description, printifyId, fabric, careInstructions, deliveryInfo, rows.id],
                  (updateErr) => {
              ...
              db.get(`SELECT id FROM products WHERE type = 'printify' AND title = ? AND (printifyId IS NULL OR printifyId = '')`, [title], (err3, legacyMatch) => {
                ...
                if (legacyMatch) {
                  ...
                  db.run(`INSERT INTO products (title, description, price, imageUrl, backImageUrl, images, stock, type, printifyId, fabric, careInstructions, deliveryInfo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [title, description, retailPrice, frontImageUrl, backImageUrl, JSON.stringify({ allImages, variantImageMap }), 999, 'printify', printifyId, fabric, careInstructions, deliveryInfo],
                    function(insertErr) {
        ...
        for (const variant of enabledVariants) {
          ...
          // Check if variant exists by (productId, printifyVariantId)
              ...
              if (existing) {
                ...
                db.all(`SELECT id FROM product_variants WHERE productId = ? AND printifyVariantId = ?`, [productId, printifyVariantId], (err2, allMatches) => {
                  ...
                  db.run(`UPDATE product_variants SET color = ?, colorHex = ?, size = ?, price = ?, cost = ?, stockQty = ?, isEnabled = 1, isAvailable = ?, imageUrl = ? WHERE id = ?`,
                    [color, colorHex, size, variantPrice, variantCost, stockQty, isAvailable, variantImageUrl, existing.id],
                    (updateErr) => {
                      if (updateErr) return reject(updateErr);
                      resolve();
                    });
                ...
                db.run(`INSERT INTO product_variants (productId, printifyVariantId, color, colorHex, size, price, cost, stockQty, isEnabled, isAvailable, imageUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`,
                  [productId, printifyVariantId, color, colorHex, size, variantPrice, variantCost, stockQty, isAvailable, variantImageUrl],
                  (insertErr) => {
                    if (insertErr) return reject(insertErr);
                    resolve();
                  });
        ...
        await new Promise((resolve, reject) => {
          db.all(`SELECT id FROM product_variants WHERE productId = ?`, [productId], (err, allVariants) => {
            if (err) return reject(err);

            if (allVariants) {
              for (const v of allVariants) {
                db.get(`SELECT printifyVariantId FROM product_variants WHERE id = ?`, [v.id], (err2, variant) => {
                  if (err2) return reject(err2);

                  if (variant && variant.printifyVariantId && !incomingPrintifyVariantIds.has(variant.printifyVariantId)) {
                    // Variant no longer in Printify - disable but don't delete
