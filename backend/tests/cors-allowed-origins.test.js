...
// CORS_ALLOWED_ORIGINS / app.use(cors({...}))) for canonical JONO domains.
//
// Exact-match allowlist, no wildcard -- confirms canonical JONO origins
// are accepted, legacy origins are rejected, unrelated origins are rejected,
// and credentials/no-Origin/preflight behavior conforms to security invariants.
...
test('CORS: legacy/unauthorized origins are rejected', async () => {
  const res = await fetch(`${baseUrl}/api/checkout/config`, {
    headers: { Origin: 'https://legacy-unauthorized.example' },
  });
  assert.equal(res.status, 500);
  assert.equal(res.headers.get('access-control-allow-origin'), null);
});
...
test('CORS: credentials are not enabled (unrelated security rule preserved, no regression)', async () => {
  const res = await fetch(`${baseUrl}/api/checkout/config`, {
    headers: { Origin: 'https://shopjono.com' },
  });
  assert.equal(res.status, 200);
...
test('CORS: preflight (OPTIONS) for a JONO origin against a real POST route is allowed', async () => {
  const res = await fetch(`${baseUrl}/api/paypal/create-order`, {
    method: 'OPTIONS',
    headers: {
      Origin: 'https://www.shopjono.com',
      'Access-Control-Request-Method': 'POST',
      'Access-Control-Request-Headers': 'content-type',
    },
  });
  assert.ok(res.status === 200 || res.status === 204, `expected a successful preflight response, got ${res.status}`);
  assert.equal(res.headers.get('access-control-allow-origin'), 'https://www.shopjono.com');
});
