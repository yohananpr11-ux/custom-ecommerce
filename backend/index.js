...
app.post('/api/test/simulate-dropship-fulfillment', async (req, res) => {
  try {
    ...
    await dbRunAsync(
      `INSERT INTO order_items (orderId, productId, variantId, quantity, price, selectedColor, selectedSize)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [orderId, product.id, variant.id, 1, product.price, variant.color, variant.size]
    );
